...
//
// workbuddy-int wraps WorkBuddy International (www.workbuddy.ai) as a cliproxy
...
const (
	providerName  = "workbuddy-int"
	authFileName  = "workbuddy-int.json"
...
func cliproxy_plugin_init(host *C.cliproxy_host_api, plugin *C.cliproxy_plugin_api) C.int {
	...
	// Start the periodic model sync (24h ticker) as soon as the plugin loads.
	modelsSyncLoop()
	return 0
}
...
func wbRegistration() registration {
	return registration{
		...
		Metadata: pluginapi.Metadata{
			...
			Logo:             "data:image/png;base64," + workbuddyIntLogoData,
		},
...
// -----------------------------------------------------------------------------
// Dynamic model sync: pull the live model list from the WorkBuddy international
// upstream /v3/config (with a logged-in credential), cache for 24h, and
// re-sync on a ticker. Falls back to the hard-coded list when unavailable.
// -----------------------------------------------------------------------------
...
const modelsSyncInterval = 24 * time.Hour

// modelSpecs is the authoritative, probed-usable model list for the current
// account. The upstream /v3/config may omit some usable models (e.g.
// deepseek-v4.1-flash, hy4-preview are callable but not listed), so this
// allowlist is the source of truth; v3/config only supplements context info.
...
// buildSpecModels converts the authoritative allowlist into ModelInfo values.
func buildSpecModels() []pluginapi.ModelInfo {
	models := make([]pluginapi.ModelInfo, 0, len(modelSpecs))
	for _, m := range modelSpecs {
		models = append(models, modelInfoFromSpec(m.id, m.name, m.contextLength, m.maxOut))
	}
	return models
}

// cachedModels returns the live-synced model list (allowlist merged with
// upstream context info) when available. Returns nil when nothing has been
// synced yet; callers then fall back to buildSpecModels() which never blocks.
func cachedModels() []pluginapi.ModelInfo {
	modelsCacheMu.Lock()
	defer modelsCacheMu.Unlock()
	return append([]pluginapi.ModelInfo(nil), modelsCache...)
}

// modelsSyncLoop refreshes the model cache every modelsSyncInterval while the
// process is resident. The first sync runs immediately after the plugin loads
// so the catalog is fresh within seconds, without blocking plugin registration.
func modelsSyncLoop() {
...
// loadModelsFromUpstream pulls the live model list from the upstream /v3/config
// using the first logged-in workbuddy-int credential found under the
// CLIProxyAPI auth directory. Returns nil on any failure.
func loadModelsFromUpstream() []pluginapi.ModelInfo {
	...
	if home != "" {
		...
		if err == nil {
			for _, e := range entries {
				name := e.Name()
				if !strings.HasPrefix(name, "workbuddy-int-") || !strings.HasSuffix(name, ".json") {
					continue
				}
	...
	models := make([]pluginapi.ModelInfo, 0, len(modelSpecs))
	// Merge: the authoritative allowlist always wins (usable models that the
	// upstream config omits, e.g. deepseek-v4.1-flash / hy4-preview, stay
	// listed). v3/config only supplements name/context/max-output when present.
	upstreamInfo := make(map[string]v3ConfigModel, len(cfg.Data.Models))
	...
	for _, spec := range modelSpecs {
		...
		seen[spec.id] = true
		if up, ok := upstreamInfo[spec.id]; ok {
			models = append(models, modelInfoFromSpec(spec.id, up.Name, up.MaxInputTok, up.MaxOutputTok))
		} else {
			models = append(models, modelInfoFromSpec(spec.id, spec.name, spec.contextLength, spec.maxOut))
		}
	}
	return models
}

// modelInfoFromSpec normalizes an upstream model entry into plugin ModelInfo.
...
func wbModels() []pluginapi.ModelInfo {
	// Fast path: use the synced allowlist if available; otherwise return the
	// authoritative allowlist immediately. Never block on upstream here — the
	// registration path must complete quickly; the background sync loop fills
	// the cache with upstream context info shortly after plugin load.
	if cached := cachedModels(); len(cached) > 0 {
		return cached
	}
	return buildSpecModels()
}
...
func sharedHTTPClient() *http.Client {
	httpClientOnce.Do(func() {
		...
		// WorkBuddy International (www.workbuddy.ai) is an overseas service; route
		// all upstream traffic through the local Clash proxy so requests are not
		// blocked or degraded by the GFW.
...
func toAuthData(sa *storedAuth) pluginapi.AuthData {
	...
	// WorkBuddy UID is the natural per-account key; without it, every login
	// would write to the same workbuddy-int.json and overwrite the previous
